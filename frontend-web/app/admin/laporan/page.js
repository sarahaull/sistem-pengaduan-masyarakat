    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-red-50 to-gray-100">
        <div className="text-center">
          ...
          <p className="mt-3 text-red-700 text-sm font-medium">Memuat laporan...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-white to-gray-100 p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="relative bg-gradient-to-r from-red-700 to-red-800 rounded-2xl shadow-lg p-6 mb-8 text-white overflow-hidden">
          ...
          <div className="relative flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-red-100 text-sm mt-1">Kelola semua laporan masyarakat dengan mudah</p>
            </div>
            <button
        ...
        {/* Statistik Cards - tema merah lembut */}
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          ...
          <div className="bg-white rounded-xl shadow-sm border-l-4 border-blue-500 p-4">
            <p className="text-xs text-blue-600 uppercase tracking-wide">Diproses</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border-l-4 border-emerald-500 p-4">
            <p className="text-xs text-emerald-600 uppercase tracking-wide">Selesai</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border-l-4 border-rose-500 p-4">
            <p className="text-xs text-rose-600 uppercase tracking-wide">Ditolak</p>
            <p className="text-2xl font-bold text-rose-700">{stats.ditolak}</p>
          </div>
        </div>

        {/* Filter & Search - dengan aksen merah */}
        <div className="bg-white rounded-xl shadow-md border border-gray-100 p-5 mb-8">
          ...
          <div className="mt-3 text-xs text-gray-400 border-t pt-3">
            Menampilkan {filteredLaporan.length} laporan dari total {laporan.length}
          </div>
        </div>

        {/* Tabel Laporan dengan header merah */}
        <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-red-50 border-b border-red-100">
                <tr>
                  <th className="text-left p-4 font-semibold text-red-800">Judul</th>
                  <th className="text-left p-4 font-semibold text-red-800">Pelapor</th>
                  <th className="text-left p-4 font-semibold text-red-800">Email</th>
                  <th className="text-left p-4 font-semibold text-red-800">Kategori</th>
                  <th className="text-left p-4 font-semibold text-red-800">Status</th>
                  <th className="text-left p-4 font-semibold text-red-800">Dibuat</th>
                  <th className="text-left p-4 font-semibold text-red-800">Aksi</th>
                </tr>
              </thead>
                    ...
                    <td colSpan={7} className="text-center py-12 text-gray-400">
                      <div className="flex flex-col items-center gap-2">
                        <div className="bg-red-50 p-3 rounded-full">
                          <FaSearch size={24} className="text-red-300" />
                        </div>
                        <p className="font-medium">Tidak ada laporan yang ditemukan</p>
                        <p className="text-xs">Coba ubah kata kunci atau filter status</p>
                      </div>
                    </td>
                  </tr>
                ) : (
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row justify-between items-center gap-3 mt-6">
            <div className="text-xs text-gray-500">
              ...
              -{" "}
              <span className="font-medium text-red-700">
                {Math.min(currentPage * itemsPerPage, filteredLaporan.length)}
              </span>{" "}
              dari <span className="font-medium text-red-700">{filteredLaporan.length}</span> laporan
            </div>
